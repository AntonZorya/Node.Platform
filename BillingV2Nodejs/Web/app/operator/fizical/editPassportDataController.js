...
function editPassportDataController($scope, dataService, modalSvc, toastr) {
    ...
    $scope.save = function () {
        if ($scope.flatList.length > 0 && !$scope.address.flat.value){
            ...
            $scope.modalItem.addressId = flat == '' ? $scope.address.house._id : $scope.address.flat._id;

            if ($scope.modalItem.controllerId && $scope.modalItem.controllerId._id)
                $scope.modalItem.controllerId = $scope.modalItem.controllerId._id;
    ...
    function GetByParentId(parentId, done) {
        dataService.get('/locationFiz/getByParentId', {parentId: parentId}).then(function (response) {
