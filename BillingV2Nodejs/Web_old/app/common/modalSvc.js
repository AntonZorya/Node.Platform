arndApplication.service('modalSvc', ['$http', '$q', '$location', '$window', '$cookieStore', 'API_HOST','$compile', modalSvc]);
...
function modalSvc($http, $q, $location, $window, $cookies, API_HOST, $compile) {
    ...
    this.resolveModal = function(modalName){
        _.find(this.modals, function(modal){ return modal.modalName==modalName}).isResolved = true;
    }

    this.isResolved = function(modalName){
        return _.find(this.modals, function(modal){ return modal.modalName==modalName}).isResolved;
    }

    this.isOkClicked = function(modalName){
        return _.find(this.modals, function(modal){ return modal.modalName==modalName}).okClicked;
    }

    this.closeModal = function(modalName){
        _.find(this.modals, function(modal){ return modal.modalName==modalName}).okClicked = true;
        $(_.find(this.modals, function(modal){ return modal.modalName==modalName}).elem).modal('hide');
    }

    this.removeModal = function(modalName){
        $(_.find(this.modals, function(modal){ return modal.modalName==modalName}).elem).remove();
        this.modals = _.reject(this.modals, function(modal){ return modal.modalName==modalName});
    }
    ...
    this.showModal = function(url, modalName, $scope){
        ...
        $http.get(url).then(function(data){
            ...
            element.modal({closable: false, onDeny    : function(){
